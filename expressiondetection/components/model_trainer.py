import os, sys
import yaml
from expressiondetection.utils.main_utils import read_yaml_file
from expressiondetection.logger import logging
from expressiondetection.exception import FaceException
from expressiondetection.entity.config_entity import ModelTrainerConfig
from expressiondetection.entity.artifacts_entity import ModelTrainerArtifact
...
class ModelTrainer:
    ...
    def initiate_model_trainer(
        ...
        try:
            if self.model_trainer_config.pre_trained:
                logging.info("Using a pretrained model in pretained folder")
                os.makedirs(self.model_trainer_config.model_trainer_dir, exist_ok=True)
                os.system(
                    f"cp pretrained_pt/best.pt {self.model_trainer_config.model_trainer_dir}/"
                )
                ...
                with open(
                    f"yolov5/models/custom_{model_config_file_name}.yaml", "w"
                ) as f:
                    yaml.dump(config, f)
